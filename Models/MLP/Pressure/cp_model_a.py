# train_cp_model_a.py
# Model A: train on XFOIL Cp data only, evaluate on RANS test set
...
def make_loaders(X, y, batch_size, val_frac=0.1):
    n = len(X)
    nv = max(1, int(n * val_frac))
    idx = np.random.permutation(n)
    Xt = torch.tensor(X[idx[nv:]], dtype=torch.float32)
    yt = torch.tensor(y[idx[nv:]], dtype=torch.float32)
    Xv = torch.tensor(X[idx[:nv]], dtype=torch.float32)
    yv = torch.tensor(y[idx[:nv]], dtype=torch.float32)
    tl = DataLoader(TensorDataset(Xt, yt), batch_size=batch_size, shuffle=True)
    vl = DataLoader(TensorDataset(Xv, yv), batch_size=batch_size)
    return tl, vl
...
def train(model, tl, vl, epochs, lr, patience, label=""):
    ...
    for ep in range(epochs):
        ...
        if wait >= patience:
            print(f"  [{label}] Early stop at epoch {ep+1} (best val: {best_loss:.6f})")
            break
...
def test_model(model):
    ...
    rmse = np.sqrt(np.mean((pred - test_cp)**2))
    return rmse, r2_score(test_cp, pred)


# ---- run ----

print("\n=== Cp Model A: XFOIL only ===")
rmses, r2s = [], []
for s in seeds:
    set_seed(s)
    tl, vl = make_loaders(xfoil_X_n, xfoil_cp_n, 128)
    m = MLP().to(DEVICE)
    m = train(m, tl, vl, 300, 1e-3, 30, label=f"A-s{s}")
...
results = {'rmse': [float(x) for x in rmses], 'r2': [float(x) for x in r2s],
           'rmse_mean': float(np.mean(rmses)), 'rmse_std': float(np.std(rmses)),
           'r2_mean': float(np.mean(r2s)), 'r2_std': float(np.std(r2s))}

with open(os.path.join(PROJECT_DIR, "Results", "cp_mlp_model_a.json"), 'w') as f:
