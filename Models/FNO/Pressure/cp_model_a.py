# train_cp_fno_model_a.py
# FNO Model A: train on XFOIL Cp only, evaluate on RANS test set
...
xfoil_X = xfoil['inputs']; xfoil_cp = xfoil['cp_distributions']; xfoil_cf = xfoil['cf_distributions']
bad = np.any((xfoil_cf > 0.1) | (xfoil_cf < -0.05), axis=1)
xfoil_X = xfoil_X[~bad]; xfoil_cp = xfoil_cp[~bad]
...
xfoil_X_n = norm.norm_in(xfoil_X); xfoil_cp_n = norm.norm_out(xfoil_cp)
...
with open(os.path.join(PROJECT_DIR, "Results", "cp_fno_model_a.json"), 'w') as f:
