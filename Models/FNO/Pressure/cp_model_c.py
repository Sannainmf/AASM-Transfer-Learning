# train_cp_fno_model_c.py
# FNO Model C: pretrain on XFOIL Cp, finetune on RANS Cp
...
class SpectralConv1d(nn.Module):
    """Fourier layer: FFT -> multiply low modes by learned weights -> inverse FFT."""
    ...
    def forward(self, x):
        ...
        B, C, L = x.shape
        x_ft = fft.rfft(x, dim=-1)  # (batch, in_channels, L//2+1)
        out_ft = torch.zeros(B, self.out_channels, L // 2 + 1,
                             dtype=torch.cfloat, device=x.device)
        # multiply low modes with learned weights
...
class FNO1D(nn.Module):
    def __init__(self, modes=32, width=64):
        ...
        # skip connections via 1x1 convs
    ...
    def forward(self, x):
        ...
        h = self.lift(spatial)                # (batch, width, 512)
        ...
        out = self.project(h)                 # (batch, 1, 512)
...
n_params = sum(p.numel() for p in FNO1D().parameters())
print(f"FNO1D parameters: {n_params:,}")

print("\n=== Cp FNO Model C: Transfer ===")
rmses, r2s = [], []
for s in seeds:
    set_seed(s)
    # pretrain on xfoil
...
with open(os.path.join(PROJECT_DIR, "Results", "cp_fno_model_c.json"), 'w') as f:
    json.dump({'rmse': [float(x) for x in rmses], 'r2': [float(x) for x in r2s],
               'rmse_mean': float(np.mean(rmses)), 'rmse_std': float(np.std(rmses)),
               'r2_mean': float(np.mean(r2s)), 'r2_std': float(np.std(r2s)),
               'architecture': 'FNO1D', 'n_params': n_params}, f, indent=2)
print("Done.")