# train_cf_fno_budget.py
# FNO data budget study for Cf: transfer vs scratch at different RANS fractions
...
shared_norm.fit(np.vstack([xfoil_X, train_X]), np.vstack([xfoil_cf, train_cf]))
xfoil_X_n = shared_norm.norm_in(xfoil_X); xfoil_cf_n = shared_norm.norm_out(xfoil_cf)
...
for frac in fractions:
    ...
    for s in seeds:
        ...
        sub_X, sub_cf = train_X[idx], train_cf[idx]
        # transfer
        tl, vl = make_loaders(xfoil_X_n, xfoil_cf_n, 128)
        m = FNO1D().to(DEVICE)
        m = train(m, tl, vl, 300, 1e-3, 30, label=f"TF-{frac:.0%}-pre-s{s}")
        tl, vl = make_loaders(shared_norm.norm_in(sub_X), shared_norm.norm_out(sub_cf), 32)
        ...
        # scratch
        sc_norm = Normalizer()
        sc_norm.fit(sub_X, sub_cf)
        tl, vl = make_loaders(sc_norm.norm_in(sub_X), sc_norm.norm_out(sub_cf), 32)
        m = FNO1D().to(DEVICE)
...
with open(os.path.join(PROJECT_DIR, "Results", "cf_fno_budget.json"), 'w') as f:
